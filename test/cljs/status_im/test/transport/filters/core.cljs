  ...
  (testing "the user is not in our contacts"
    (testing "the user is not in any group chats or 1-to1-"
      (testing "it removes the filters"
        (let [fx (transport.filters/stop-listening {:db {:transport/filters
                                                         {chat-id ["a"]}}}
                                                   chat-id)]
          (is fx)
          (is (= fx {:filters/remove-filters ["a"]})))))
...
(deftest add-filter-to-db
  (with-redefs [mailserver.topics/upsert (fn [{:keys [db]} r] {:db (assoc db :upsert r)})]
    (let [expected {:db {:transport/filter-ids #{"filter-id"}
                         :transport/filters {"chat-id" [{:filter-id "filter-id"
                                                         :discovery? false
                                                         :chat-id "chat-id"
                                                         :negotiated? false
                                                         :topic "topic"}]}
