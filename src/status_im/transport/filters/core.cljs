...
(defn loaded?
  ...
  [{:keys [db]} {:keys [filter-id] :as f}]
  (let [filter-ids (get db :transport/filter-ids #{})]
    (contains? filter-ids filter-id)))

(fx/defn set-raw-filter
  "Update filter ids cached and set filter in the db"
  [{:keys [db]} {:keys [chat-id filter-id] :as filter}]
  (let [filter-ids (get db :transport/filter-ids #{})]
    {:db (-> db
             (assoc :transport/filter-ids (conj filter-ids filter-id))
             (update-in [:transport/filters chat-id] conj filter))}))

(fx/defn unset-raw-filter
  "Update filter ids cached and set filter in the db"
  [{:keys [db]} {:keys [chat-id filter-id] :as filter}]
  (let [filter-ids (get db :transport/filter-ids #{})
        remove-fn  (fn [filters] (remove #(= filter-id (:filter-id %)) filters))]
    {:db (-> db
             (assoc :transport/filter-ids (disj filter-ids filter-id))
             (update-in [:transport/filters chat-id] remove-fn))}))

(fx/defn add-filter-to-db
  "Set the filter in the db and upsert a mailserver topic"
  [cofx filter]
  (when-not (loaded? cofx filter)
    (fx/merge cofx
              (set-raw-filter filter)
              (upsert-mailserver-topic filter))))

(fx/defn remove-filter-from-db
  "Remove the filter from the db"
  [cofx filter]
  (fx/merge cofx
...
(fx/defn handle-negotiated-filter
  ...
  (let [filter-ids      (:transport/filter-ids db)
        processed-filters (map responses->filters filters)
        new-filters     (filter #(not (filter-ids (:filter-id %))) processed-filters)]
...
(fx/defn stop-filters
  "Stop all filters"
  [{:keys [db]} callback]
  (let [filters (map :filter (flatten
                              (vals
                               (:transport/filters db))))]
...
(fx/defn load-chat
  ...
  [{:keys [db]} chat-id]
  (when-not (get-in db [:transport/filters chat-id])
...
(fx/defn load-contact
  ...
  [{:keys [db]} contact]
  (when-not (get-in db [:transport/filters (:public-key contact)])
...
(fx/defn load-member
  ...
  [{:keys [db]} public-key]
  (when-not (get-in db [:transport/filters public-key])
